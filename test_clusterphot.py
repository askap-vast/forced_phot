...
for j in range(len(separations)):
    ...
    flux_recover,flux_err_recover,chi2_recover,DOF_recover=FP.measure(P_inj,cluster_threshold=None)
    # recover assuming  clusters
    flux_cluster,flux_err_cluster,chi2_cluster,DOF_cluster=FP.measure(P_inj,cluster_threshold=2*(s/FP.pixelscale).decompose().value)
    ...
    if j==0:
        ...
        plt.errorbar(flux_inj,(flux_cluster-flux_inj),yerr=flux_err_cluster,fmt='s')
...
plt.plot(FP.BMAJ.to(u.arcsec)*np.array([1,1]),plt.gca().get_ylim(),':',label='BMAJ')
